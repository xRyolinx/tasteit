...
async function add_el(template, add) {
    // BDD de plats
    let response = await fetch("/dishes_list?id=" + id + "&n=" + add + query);
    ...
    for (let i in plats)
    {
        ...
        restau.innerHTML = plats[i]["restaurant"];
        // Prix
        let price = el_txt.children[2].children[0];
        price.innerHTML = plats[i]["price"] + " DA";
...
async function last_async() {
    let response = await fetch("/dishes_list?t=0" + query);
